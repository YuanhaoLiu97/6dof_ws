...
class StaticTFBroadcaster(Node):
    def __init__(self, name):
        ...
        self.declare_parameters(
            namespace='',
            parameters=[
                ('quat',rclpy.Parameter.Type.DOUBLE_ARRAY),
                ('tran',rclpy.Parameter.Type.DOUBLE_ARRAY)
            ]
        )                                                    
        self.tf_broadcaster = StaticTransformBroadcaster(self)                  # 创建一个TF广播器对象
        quat = self.get_parameter('quat').get_parameter_value().double_array_value
        tran = self.get_parameter('tran').get_parameter_value().double_array_value
        self.get_logger().info('trans %f, %f, %f' %(tran[0], tran[1], tran[2]))
        self.get_logger().info('quaternion %f, %f, %f, %f' %(quat[0], quat[1], quat[2], quat[3]))
        ...
        static_transformStamped.child_frame_id  = 'camera'                       # 设置一个坐标变换的目标坐标系
        static_transformStamped.transform.translation.x = tran[0]                  # 设置坐标变换中的X、Y、Z向的平移
